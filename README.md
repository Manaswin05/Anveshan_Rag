...
[ Upload Document ] -> [ Dynamic Chunking & Indexing ] -> [ Chroma DB + BM25 ]
                                                                 |
...
pydantic
  ...
  📖 User GuideAPI Configuration: Enter your Gemini API Key in the left sidebar control panel. Optionally provide a LangSmith key for pipeline tracing.  
  Document Ingestion: Upload a PDF, DOCX, TXT, or CSV file and click 🚀 Process.  
  Workspace Analysis:
  View dynamic metadata metrics (pages/rows, chunk count, chunk size, overlap).  
  Expand the Executive Summary tab to review key topics.  
  Interactive Chat:
  Use quick-suggestion prompt chips or enter custom questions in the chat box.  
  ...
  Deploying via DockerBuild Docker Image:Bashdocker build -t hybrid-rag-workstation .
Run Container:
Bash
